                ...
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300",
                  {
                    "text-gray-950 dark:text-gray-200":
                      activeSection === ( language == 'english' ? link.nameEn : link.namePt ),
                  }
                )}
                href={link.hash}
                onClick={() => {
                  setActiveSection(link.nameEn);
                  setTimeOfLastClick(Date.now());
                }}
              >
                { language == 'english' ? link.nameEn : link.namePt }

                {(language == 'english' ? link.nameEn : link.namePt) === activeSection && (
                  <motion.span
